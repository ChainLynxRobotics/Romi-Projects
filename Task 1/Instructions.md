...
Use the WPILib VSCode extension to create a new Romi template, to do so first press `Ctrl + Shift + P` then search for `Create a new project`, then click it and select the following options before creating the project, Project type `Template`, language `java`, Project base `Romi - Command Robot`, team number `8248`, Enable Desktop Support `true`, you will also need to select where the project will be located, and can name it whatever you want. Finally you can press the generate project button.
